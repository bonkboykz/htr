...
| Статика/время   | `duration_s` (int, сек) | `45`   | `45s` / `1m 30s` |

Для статики (планка, растяжка) логируется `duration_s` (секунды), `weight_g = 0`. Тогда
`getVolumeStats` отдаёт и `durationS` (суммарное время удержания) наряду с `volumeG`, а
`getProgression` авто-переключается в `metric: "duration"` (тренд по лучшему удержанию, e1RM = 0).
