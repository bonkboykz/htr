...
class ApiException implements Exception {
  ...
  final int? status;
  ApiException(this.message, {this.code = 'ERROR', this.status});
  bool get isUnauthorized => status == 401;
  @override
  String toString() => message;
}

/// Thin Dio wrapper over the HTR REST API. Features call get/post/patch/delete
/// and parse their own JSON. The Bearer token is attached per-request from
/// [TokenStorage] (qmeta pattern), so changing the key takes effect immediately.
class ApiClient {
  final Dio dio;
